...
/* ---------- Planet data ----------
   Every planet revolves around the sun on its own elliptical path.
   - day ellipse: orbit radius, so planets circle the sun by day
   - the existing day/night progress smoothly fades planets OUT from
     their current orbital position as night falls (never a snap) */
...
function renderPlanets() {
  ...
  // Planets stay visible through day & sunset, then phase out from their
  // current orbital position as night deepens.
  const fade = smoothstep(0.5, 0.95, progress);

  parallaxX += (parallaxTargetX - parallaxX) * 0.06;
  parallaxY += (parallaxTargetY - parallaxY) * 0.06;

  for (const planet of planetInstances) {
    const point = ellipsePoint(planet.orbit.rx * base, planet.orbit.ry * base, planet.orbit.tilt, planet.angle);

    // Planets revolve around the sun; depth parallax shifts them with the cursor.
    const px = centerX + point.x + parallaxX * planet.depth * MAX_PARALLAX;
    const py = centerY + point.y + parallaxY * planet.depth * MAX_PARALLAX;

    // Brighter for closer planets; fade out (from their current position) at night.
    const opacity = (0.55 + 0.45 * planet.depth) * (1 - fade);
